...
class CheapPriceDataUpdateCoordinator(DataUpdateCoordinator):
    ...
    def update_trigger_config(self) -> None:
        ...
        trigger_time = config.get(CONF_CHEAP_PRICE_UPDATE_TRIGGER, DEFAULT_CHEAP_PRICE_UPDATE_TRIGGER)
    ...
    async def _async_update_data(self) -> Any:
        ...
        try:
            ...
            if not main_data:
                _LOGGER.warning("No data available from main coordinator for cheap price calculation")
                return None
            ...
            _LOGGER.debug("Cheap price data updated with %d ranges", len(cheap_data.get("cheap_ranges", [])))
            return cheap_data
    ...
    def _get_trigger_time(self) -> str:
        ...
        config = {**self.config_entry.data, **self.config_entry.options}
        return config.get(CONF_CHEAP_PRICE_UPDATE_TRIGGER, DEFAULT_CHEAP_PRICE_UPDATE_TRIGGER)
    ...
    def _analyze_cheap_prices(self, main_data: dict) -> dict[str, Any]:
        ...
        try:
            ...
            if cheap_df.empty:
                ...
                return {"cheap_ranges": [], "analysis_info": {"min_price": min_price, "threshold_percent": threshold_percent}}
    ...
    def get_current_cheap_price(self) -> float | None:
        ...
        for range_data in self.data["cheap_ranges"]:
            start_time = datetime.datetime.fromisoformat(range_data["start_time"].replace("Z", "+00:00"))
            end_time = datetime.datetime.fromisoformat(range_data["end_time"].replace("Z", "+00:00"))
