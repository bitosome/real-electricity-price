...
def datetime_serializer(obj) -> str | dict | list:
...
async def async_setup_entry(
    ...
    _LOGGER.critical(
        "CRITICAL DEBUG: async_setup_entry called in sensor.py!"
    )  # Should appear in logs
    _LOGGER.warning(
        f"SENSOR SETUP ENTRY CALLED - Starting async_setup_entry for entry: {entry.entry_id}"
    )
    ...
    for entity_description in ENTITY_DESCRIPTIONS:
        ...
        if entity_description.key == "real_electricity_price":
            ...
            coord = (
                cheap_price_coordinator  # Use cheap price coordinator for this sensor
            )
            _LOGGER.debug(
                f"Setting up new sensor: {entity_description.key} with type {sensor_type}"
            )
        ...
        _LOGGER.debug(
            f"Creating sensor entity: {entity_description.key} -> {sensor_type}"
        )
        entities.append(
...
class RealElectricityPriceSensor(RealElectricityPriceEntity, SensorEntity):
    ...
    def __init__(
        ...
        | CheapPriceDataUpdateCoordinator,
        ...
        _LOGGER.debug(
            "Initializing sensor: %s (type: %s)", entity_description.key, sensor_type
        )
    ...
    def _create_price_components(self, config: dict) -> dict[str, Any]:
        ...
        return {
            ...
            "tax_info": {
                "vat_percentage": self._get_price_config_value(
                    config, CONF_VAT, VAT_DEFAULT
                ),
                "vat_applied_to": {
                    "nord_pool_price": config.get(
                        CONF_VAT_NORD_POOL, VAT_NORD_POOL_DEFAULT
                    ),
                    f"{grid_name.lower()}_electricity_excise_duty": config.get(
                        CONF_VAT_GRID_ELECTRICITY_EXCISE_DUTY,
                        VAT_GRID_ELECTRICITY_EXCISE_DUTY_DEFAULT,
                    ),
                    f"{grid_name.lower()}_renewable_energy_charge": config.get(
                        CONF_VAT_GRID_RENEWABLE_ENERGY_CHARGE,
                        VAT_GRID_RENEWABLE_ENERGY_CHARGE_DEFAULT,
                    ),
                    f"{grid_name.lower()}_transmission_night": config.get(
                        CONF_VAT_GRID_TRANSMISSION_NIGHT,
                        VAT_GRID_TRANSMISSION_NIGHT_DEFAULT,
                    ),
                    f"{grid_name.lower()}_transmission_day": config.get(
                        CONF_VAT_GRID_TRANSMISSION_DAY,
                        VAT_GRID_TRANSMISSION_DAY_DEFAULT,
                    ),
                    f"{supplier_name.lower()}_renewable_energy_charge": config.get(
                        CONF_VAT_SUPPLIER_RENEWABLE_ENERGY_CHARGE,
                        VAT_SUPPLIER_RENEWABLE_ENERGY_CHARGE_DEFAULT,
                    ),
                    f"{supplier_name.lower()}_margin": config.get(
    ...
    def native_value(self) -> Any:
        ...
        if self._sensor_type == SENSOR_TYPE_LAST_CHEAP_CALCULATION:
        ...
        if self._sensor_type in (SENSOR_TYPE_CURRENT_PRICE, SENSOR_TYPE_HOURLY_PRICES):
    ...
    def _get_last_cheap_calculation_value(self) -> datetime.datetime:
        ...
        _LOGGER.debug(
            "Last cheap calculation sensor: coordinator.data = %s",
    ...
    def _get_current_tariff_value(self) -> str:
        ...
        _LOGGER.debug(
            "Holiday/Weekend check: is_holiday = %s, is_weekend = %s",
            is_holiday,
            is_weekend,
        )
        ...
        if is_holiday or is_weekend:
            _LOGGER.debug(
                "Returning 'night' tariff due to %s",
                "holiday" if is_holiday else "weekend",
            )
            return "night"
        ...
        _LOGGER.debug(
            "Local time %s:00 -> %s tariff (tz=%s)", local_hour, tariff, tz_name
        )
    ...
    def extra_state_attributes(self) -> dict[str, Any]:
        ...
        if self._sensor_type == SENSOR_TYPE_HOURLY_PRICES:
    ...
    def _get_special_sensor_attributes(self) -> dict[str, Any]:
        ...
        if self._sensor_type == SENSOR_TYPE_LAST_CHEAP_CALCULATION:
            ...
            calculation_info = {
                ...
                "coordinator_available": self.coordinator.last_update_success,
                "trigger_time": self.coordinator.data.get("trigger_time")
    ...
    def _get_current_price_attributes(self) -> dict[str, Any]:
        ...
        for data_key in self.coordinator.data:
            ...
            for price in hourly_prices:
                try:
                    start = datetime.fromisoformat(price["start_time"])
                    end = datetime.fromisoformat(price["end_time"])
                    if start <= now < end:
        ...
        current_hour_info = {
            "date": current_date,
            "hour_start": current_hour_data["start_time"]
            if current_hour_data
            else None,
            "hour_end": current_hour_data["end_time"] if current_hour_data else None,
            "nord_pool_price": self._round_price(current_hour_data["nord_pool_price"])
            if current_hour_data and current_hour_data["nord_pool_price"] is not None
            else None,
    ...
    def _get_hourly_prices_attributes(self) -> dict[str, Any]:
        ...
        for data_key in sorted(self.coordinator.data.keys()):
            ...
            hourly_prices = day_data.get("hourly_prices", [])
            data_available = day_data.get(
                "data_available", True
            )  # Default to True for backward compatibility
            date = day_data.get("date", "unknown")

            _LOGGER.debug(
                f"Found {len(hourly_prices)} hourly prices for {data_key} (date: {date}, available: {data_available})"
            )
    ...
    def _get_cheap_prices_attributes(self) -> dict[str, Any]:
        ...
        if self._sensor_type == SENSOR_TYPE_CHEAP_PRICES:
            if hasattr(self.coordinator, "data") and self.coordinator.data:
                        ...
                        if start_time > now:
                            ...
                            if min_time_diff is None or time_diff < min_time_diff:
                                ...
                                next_cheap_info = {
                                    ...
                                    "average_price": self._round_price(
                                        range_data["avg_price"]
                                    ),
                                }
                ...
                return {
                    "cheap_price_ranges": json.dumps(
                        cheap_ranges_serializable, indent=2
                    ),
    ...
    def _analyze_cheap_prices(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            _LOGGER.exception("Error analyzing cheap prices: %s", e)
            return []
    ...
    def _get_price_analysis_info(self) -> dict[str, Any]:
        ...
        try:
            ...
            _LOGGER.exception("Error getting price analysis info: %s", e)
            return {}
